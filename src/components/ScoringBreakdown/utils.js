...
export function getScoring(participants, gameStats) {
  ...
  participants.forEach(({ name, players }) => {
    scoring[name] = players.map((playerName) => {
      // Get team and points for player
      const playerStats = gameStats.filter((s) => s.player === playerName)
      ...
      if (playerStats.length === 0) {
        // This would happen if the player hasn't logged any stats and isn't in haventPlayedYet
        return { name: playerName, team: '', points: 0 }
      }

      return {
        name: playerName,
        team: playerStats[0].team,
        points: d3.sum(playerStats.map((s) => s.points)),
      }
    })
  })
